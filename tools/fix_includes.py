...
# License.
# See https://github.com/include-what-you-use/include-what-you-use/blob/master/LICENSE.TXT
# for details.
#
##===----------------------------------------------------------------------===##
...
def main(argv):
    ...
    if (
        ...
        and not flags.separate_project_includes.startswith("<")  # 'special' vals
        and not flags.separate_project_includes.endswith(os.path.sep)
    ...
    if flags.sort_only:
        ...
        process_ret = ProcessIWYUOutput(
            sys.stdin, files_to_modify, flags, cwd=os.getcwd()
        )
        if flags.dry_run:
